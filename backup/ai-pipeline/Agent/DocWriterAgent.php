 ...
 * @phpstan-type DocPlan array<string, mixed>
 */
final readonly class DocWriterAgent implements AgentInterface
{
    private const int MAX_SOURCE_BYTES_PER_FILE = 8000;

    private const int MAX_SOURCE_TOTAL_BYTES = 60000;

    /**
     * @return list<DocPlan>
     */
    private const array DEFAULT_PLAN = [
     ...
     * @param  array<mixed>  $sourceContents
...
';
    ...
    private function readSourceBlock(array $files, array $sourceContents): string
    {
        ...
        foreach ($files as $file) {
            $candidate = $sourceContents[$file] ?? $this->readSourceFile($file);
            $contents = is_string($candidate) ? $candidate : '';
